...
{ // "char *" must be on a separate line, brace must be on a new line
    return NULL;
}
...
main(void)
{
    ...
    // Rule 2.3
    if (2 > 1 && 1 < 2) { // needs one extra tab
        1 == 1;
    }
...
LABEL:          // must be lower_case
    ...
    // Rule 4.4.2
    double summ = 0.0; // must be on previous line
    ...
    // Rule 4.6.12
    // Использование оператора goto не рекомендуется. Допускается использование оператора goto при обработке ошибок и
    // при выходе из вложенных циклов. Допускаются только переходы вперед по тексту программы.
...
#define BUFFER_SIZE 1024       // must be enum
    ...
    // Rule 5.9
    // Использование функции malloc не рекомендуется. Допускается использовать malloc при реализации расширяемых
    // массивов, если предполагается выполнять realloc. Во всех остальных случаях используйте функцию calloc.
    // Запрещается использовать malloc при выделении памяти под элементы списков, деревьев и прочих подобных структур.
    ...
    // Rule 5.13
    // Вспомогательные функции общего назначения (такие, как getline, функции работы со списками и т. д.) не должны
    // ничего выводить на stdout или stderr. В случае возникновения ошибки такие функции должны, как правило, возвращать
    // специальное значение.

    // Rule 5.14
    // Глобальные переменные должны использоваться только при необходимости. Передача параметров предпочтительнее
